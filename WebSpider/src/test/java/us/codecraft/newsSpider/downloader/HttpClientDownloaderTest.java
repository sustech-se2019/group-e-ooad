...
import us.codecraft.newsSpider.model.HttpRequestBody;
import us.codecraft.newsSpider.proxy.Proxy;
import us.codecraft.newsSpider.proxy.SimpleProxyProvider;
import us.codecraft.newsSpider.selector.Html;
import us.codecraft.newsSpider.utils.CharsetUtils;
...
public class HttpClientDownloaderTest {
    ...
    public void test_selectRequestMethod() throws Exception {
        ...
        server.get(eq(query("q"), "newsSpider")).response("get");
        server.post(eq(form("q"), "newsSpider")).response("post");
        server.put(eq(form("q"), "newsSpider")).response("put");
        server.delete(eq(query("q"), "newsSpider")).response("delete");
        server.request(and(by(method("HEAD")),eq(query("q"), "newsSpider"))).response(header("method","head"));
        server.request(and(by(method("TRACE")),eq(query("q"), "newsSpider"))).response("trace");
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                request.setUrl("http://127.0.0.1:" + port + "/search?q=newsSpider");
                request.setMethod(HttpConstant.Method.GET);
                Map<String,Object> params = new HashedMap();
                params.put("q","newsSpider");
    ...
    public void test_set_request_cookie() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(cookie("cookie"), "cookie-newsSpider")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                ...
                request.addCookie("cookie","cookie-newsSpider");
    ...
    public void test_disableCookieManagement() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(not(eq(cookie("cookie"), "cookie-newsSpider"))).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
    ...
    public void test_set_request_header() throws Exception {
        HttpServer server = httpServer(13423);
        server.get(eq(header("header"), "header-newsSpider")).response("ok");
        Runner.running(server, new Runnable() {
            @Override
            public void run() throws Exception {
                HttpClientDownloader httpClientDownloader = new HttpClientDownloader();
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                request.addHeader("header","header-newsSpider");
                Page page = httpClientDownloader.download(request, Site.me().toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    ...
    public void test_set_site_header() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Page page = httpClientDownloader.download(request, Site.me().addHeader("header","header-newsSpider").toTask());
                assertThat(page.getRawText()).isEqualTo("ok");
            }
        });
    }
    ...
    public void test_set_site_cookie() throws Exception {
        ...
        Runner.running(server, new Runnable() {
            ...
            public void run() throws Exception {
                ...
                Request request = new Request();
                request.setUrl("http://127.0.0.1:13423");
                Site site = Site.me().addCookie("cookie", "cookie-newsSpider").setDomain("127.0.0.1");
